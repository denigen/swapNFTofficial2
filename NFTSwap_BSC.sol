...
/**
 * @title NFTSwap
 * @dev Secure peer-to-peer NFT swap contract optimized for BSC mainnet with BNB payments
 */
contract NFTSwap_BSC is ReentrancyGuard, Pausable, Ownable {
    // Structs
    struct NFTDetails {
    ...
    struct TokenDetails {
        // Use "BNB" for native token; other symbols are treated as ERC20 and resolved via tokenAddresses mapping
        string symbol;
        uint256 amount;
    }
    ...
    // Fees (denominated in BNB wei) and escrow tracking
    // Default: 0.003 BNB per executed swap
    uint256 public swapFee = 0.003 ether;

    // Tracks BNB deposited by makers for active orders so owner cannot withdraw user funds
    uint256 private reservedBnb;
    ...
    /**
     * @dev Creates a new swap order with optional BNB payment
     */
    function createSwapOrder(
        ...
        require(makerNFTs.length > 0 || makerToken.amount > 0, "Must offer NFTs or BNB");
        require(takerNFTs.length > 0 || takerToken.amount > 0, "Must request NFTs or BNB");
        ...
        if (makerToken.amount > 0) {
            if (keccak256(bytes(makerToken.symbol)) == keccak256(bytes("BNB"))) {
                require(msg.value == makerToken.amount, "Incorrect BNB amount sent");
                reservedBnb += makerToken.amount;
            } else {
                address tokenAddr = tokenAddresses[makerToken.symbol];
                require(tokenAddr != address(0), "Unsupported token symbol");
                require(msg.value == 0, "BNB not expected for ERC20");
                require(IERC20(tokenAddr).transferFrom(msg.sender, address(this), makerToken.amount), "ERC20 escrow failed");
            }
        } else {
            require(msg.value == 0, "Unexpected BNB sent");
        }
    ...
    function executeSwap(bytes32 orderId) external payable nonReentrant {
        ...
        // Validate and collect swap fee from taker (in addition to any taker BNB payment)
        uint256 requiredMsgValue = swapFee;
        if (order.takerToken.amount > 0) {
            if (keccak256(bytes(order.takerToken.symbol)) == keccak256(bytes("BNB"))) {
        ...
        require(msg.value == requiredMsgValue, "Incorrect BNB amount");

        // Forward taker's BNB payment (excluding fee) to maker if required
        if (order.takerToken.amount > 0 && keccak256(bytes(order.takerToken.symbol)) == keccak256(bytes("BNB"))) {
            (bool success, ) = order.maker.call{value: order.takerToken.amount}("");
            require(success, "BNB transfer to maker failed");
        }

        // Handle maker's BNB payment
        if (order.makerToken.amount > 0) {
            if (keccak256(bytes(order.makerToken.symbol)) == keccak256(bytes("BNB"))) {
                // Release reserved BNB to taker
                reservedBnb -= order.makerToken.amount;
                (bool success, ) = msg.sender.call{value: order.makerToken.amount}("");
                require(success, "BNB transfer to taker failed");
            } else {
    ...
    function availableFees() public view returns (uint256) {
        ...
        if (bal <= reservedBnb) return 0;
        return bal - reservedBnb;
    }
    ...
    // Receive function to accept BNB payments
