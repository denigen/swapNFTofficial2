...
contract NFTSwap_BSC is ReentrancyGuard, Pausable, Ownable {
    ...
    struct TokenDetails {
        string symbol;    // BNB, ETH, APE, or HYPE
        uint256 amount;
    }
    ...
    function createSwapOrder(
        ...
        // Verify BNB payment if offering and reserve it until execution/cancellation
        if (makerToken.amount > 0) {
            require(
                keccak256(bytes(makerToken.symbol)) == keccak256(bytes("BNB")),
                "Only BNB token supported"
            );
    ...
    function executeSwap(bytes32 orderId) external payable nonReentrant {
        ...
        if (order.takerToken.amount > 0) {
            require(
                keccak256(bytes(order.takerToken.symbol)) == keccak256(bytes("BNB")),
                "Only BNB token supported"
            );
        ...
        if (order.makerToken.amount > 0) {
            require(
                keccak256(bytes(order.makerToken.symbol)) == keccak256(bytes("BNB")),
                "Only BNB token supported"
            );
